...
#                osddir='/var/lib/ceph/osd/%s-%d'%(clusterid, osdnum)
    ...
    def check_health(self, check_list=None, logfile=None, recstatsfile=None):
        ...
        logger.info(f"Waiting for {self.health_wait} seconds for the pool to get ready ")
        ...
        check_list = ["degraded", "peering", "recovery_wait", "stuck", "inactive", "unclean", "recovery", "stale","activating","creating"]
        if recstatsfile:
        ...
        while True:
            result = subprocess.run("ceph status",capture_output=True,shell=True,text=True)
            # print(f"\n\n{result.stdout}\n\n")
            self.log_recovery_stats(recstatsfile)
            if not any(x in result.stdout for x in check_list):
                break
            if not "HEALTH_OK" in result.stdout:
                continue
            else:
                ret = ret + 1
    ...
    # Here I create all EC profiles at once
    def make_profiles(self):
        ...
        for name, profile in list(erasure_profiles.items()):
            ...
            subprocess.run(f"ceph osd erasure-code-profile set {name} k={k} m={m} plugin={plugin} "
                           f"technique=reed_sol_van",shell=True,text=True)

    def mkpool(self, name, profile_name, application, base_name=None):
        pool_profiles = self.config.get('pool_profiles', {'default': {}})
        ec_profiles = self.config.get('erasure_profiles', {'default': {}})
        ...
        ec_name = ec_profiles.get(erasure_profile,'')
        ...
        if replication and replication == 'erasure':
            subprocess.run(f"ceph osd pool create {name} {pg_size} {pgp_size} erasure {erasure_profile}",shell=True,text=True)
            ...
            if ec_overwrites is True:
                subprocess.run(f"ceph osd pool set {name} allow_ec_overwrites true",shell=True,text=True)
            ...
            subprocess.run(f"ceph osd pool create {name} {pg_size} {pgp_size}",shell=True,text=True)
        ...
        subprocess.run(f"ceph osd pool application enable {name} {application}",shell=True,text=True)
        ...
        if replication and replication.isdigit():
            ...
            logger.info(f"Setting pool {name} size")
            subprocess.run(f"ceph osd pool set {name} size {replication} {yes_flag}",shell=True,text=True)
            ...
            #             continue_if_error=False).communicate()
            logger.info(f"Setting pool {name} min_size")
            subprocess.run(f"ceph osd pool set {name} min_size {pool_min_repl_size} {yes_flag}",shell=True,text=True)
            time.sleep(2)
    ...
    def rmpool(self, name, profile_name):
        ...
        cmd = f"{self.ceph_cmd} -c {self.tmp_conf} osd pool delete {name} {name} --yes-i-really-really-mean-it"
        # print(f"\n+++++RUNNING SUBPROCESS INSTEAD OF PDSH+++++\n")
